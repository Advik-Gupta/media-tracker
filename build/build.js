...
function sweepPages() {
  ...
  for (const u of universes) wanted.add(pagePath(u.kind, u.id));
...
let pages = 0;
for (const u of universes) {
  /* Shows read better as a season grid than as a timeline, so they use their
     own template - but only once their episode data has been fetched. */
  /* Anything with episode data reads better as a season grid than a
     timeline, whichever vault it lives in. */
  const hasEpisodes = fs.existsSync(path.join(DATA, "series", `${u.id}.js`));
  const EPISODIC = new Set(["show", "anime"]);
  const tpl = EPISODIC.has(u.kind) && hasEpisodes ? seriesTpl : uniTpl;
  writeHtml(pagePath(u.kind, u.id), ejs.render(tpl, { u, vault: vaultOf(u), base: PAGE_BASE }));
  pages++;
}
